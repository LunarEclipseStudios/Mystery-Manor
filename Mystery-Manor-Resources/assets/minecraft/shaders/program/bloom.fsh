...
// Credit to bradleyq for bloom shader. Link: https://github.com/bradleyq/shader-toolkit
...
void main() {
    ...
    for (int ii = 0; ii < TAPS; ++ii) {
        ...
        bloomAccumulator += colortmp / clamp(exp((1.0 - lumtmp) * 5.0), 1.0, 50.0) * (1.0 - pow(length(poissonDisk[ii]), 4.0));
    }
    bloomAccumulator /= float(TAPS);
    bloomAccumulator *= STRENGTH;

    fragColor = vec4(OutTexel + bloomAccumulator, 1.0);
    vec3 color = mix(fragColor.rgb, vec3(length(fragColor.rgb) / sqrt(3.0)), luminance(fragColor.rgb) * 0.25);
    fragColor.rgb = czm_saturation(color, 1.1);
}
